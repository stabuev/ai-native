...
4. **BUILD IT** — реализация с нуля в `code/`, запускается на CPU офлайн.
...
- В `code/` рабочий код и проходящий `test_*.py`.
- В `outputs/` обещанный артефакт (непустой).
- `pytest <lesson>/code` зелёный; код без платных ключей (исключения помечены `**Requires:**`).
