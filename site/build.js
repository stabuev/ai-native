...
function lessonSlug(dir) {   // dir = "phases/<PP-phase>/<LL-lesson>" → slug без номера урока
...
function injectFiles(html, collected, exercise) {
  ...
  const isSample = (it) => it.kind === "code" && /(^|\/)sample[_-]/.test(it.label);
  ...
  const groups = exercise
    ? [
        ...
        { pick: (it) => isSample(it),                     title: "Учебные данные", collapsed: false,
          note: "Этот безопасный пример можно использовать вместо рабочего файла." },
        { pick: (it) => isTest(it),                       title: "Тесты — это твоё ТЗ", collapsed: false, note: "" },
        { pick: (it) => it.kind === "code" && !isTest(it) && !isStarter(it) && !isSample(it), title: "Эталонное решение", collapsed: true,
    ...
    : [
        { pick: (it) => it.kind === "code",   title: "Код урока", collapsed: false, note: "" },
        { pick: (it) => it.kind === "output", title: "Артефакт",   collapsed: false, note: "" },
        { pick: (it) => it.kind === "other",  title: "Связанные файлы",       collapsed: false, note: "" },
      ];
  ...
  if (exercise) {
    sec += `<div class="three-ways"><h3>Три пути пройти упражнение</h3><ol>`
      ...
      + `</ol><p class="muted">Тесты — общий критерий приёмки на любом пути. Если делегируешь ИИ, `
      + `реши, что отдаёшь; опиши задачу и критерии; проверь результат; сохрани ответственность за итог.</p></div>`;
  }
...
<head>
  ...
  <script type="text/javascript">
      ...
      ym(86601085, 'init', {clickmap:true, referrer: document.referrer, url: location.href, accurateTrackBounce:true, trackLinks:true});
  </script>
  <noscript><div><img src="https://mc.yandex.ru/watch/86601085" style="position:absolute; left:-9999px;" alt="" /></div></noscript>
  <!-- /Yandex.Metrika counter -->
</head>
